...
.get(utils.checkUser(), function(req, res, next) {
	...
	recipe.fetchRecipe(req.user.alexaID, recipeId, function(err, recipe) {
		if (err) {
...
.post(utils.checkUser(), function(req, res, next) {
	recipe.createRecipe(req.user.alexaID, req.body, function(err, recipe) {
		if (err) {
			return next(new HTTPError.InternalServerError('' + err));
		}
...
.delete(utils.checkUser(), function(req, res, next) {
	...
	recipe.removeRecipe(req.user.alexaID, recipeId, function(err) {
		if (err) {
			return next(new HTTPError.InternalServerError('' + err));
		}
...
.get(utils.checkUser(), function(req, res) {
	recipe.fetchAllRecipes(req.user.alexaID, function(err, recipes) {
		if (err) {
			return next(new HTTPError.InternalServerError('' + err));
		}
