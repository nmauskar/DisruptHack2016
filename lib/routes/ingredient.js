...
.get(utils.checkUser(), function(req, res, next) {
	...
	recipe.fetchIngredient(req.user._id, ingredientId, function(err, ingredient) {
		if (err) {
...
.post(utils.checkUser(), function(req, res, next) {
	recipe.createIngredient(req.user._id, req.body, function(err, ingredient) {
		if (err) {
			return next(new HTTPError.InternalServerError('' + err));
		}
...
.delete(utils.checkUser(), function(req, res, next) {
	...
	recipe.removeIngredient(req.user._id, ingredientId, function(err) {
		if (err) {
			return next(new HTTPError.InternalServerError('' + err));
		}
...
router.route('/all')
.get(utils.checkUser(), function(req, res, next) {
	recipe.fetchAllIngredients(req.user._id, function(err, ingredients) {
		if (err) {
			return next(new HTTPError.InternalServerError('' + err));
		}
