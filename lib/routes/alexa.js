...
app.intent('GetMealsIntent', function(request, response){
	var alexaID = request.data.session.user.userId;

	db.users.findOne({ alexaID: alexaID }, function(err, user) {
		if (err) {
			console.error('Alexa', err.stack || err.message || err.toString());
			return response.say('Internal server error, could not complete task.');
		}

		if (!user) {
			return response.say('Could not find user.');
		}

		recipeController.fetchAllRecipes(user._id, function(err, recipes){
			if (err) {
				console.error('Alexa', err.stack || err.message || err.toString());
				return response.say('Internal server error, could not complete task.');
			}
...
app.intent('GoneBadIntent', function(request, response) {
	...
	db.users.findOne({ alexaID: alexaID }, function(err, user) {
		...
		ingredientController.fetchBadIngredients(user._id, function(err, ingredients){
			if (err) {
				console.error('Alexa', err.stack || err.message || err.toString());
				return response.say('Internal server error, could not complete task.');
			}
...
app.intent('TellAllIntent', function(request, response) {
	...
	db.users.findOne({ alexaID: alexaID }, function(err, user) {
		...
		ingredientController.fetchAllIngredients(user._id, function(err, ingredients){
			if (err) {
				console.error('Alexa', err.stack || err.message || err.toString());
				return response.say('Internal server error, could not complete task.');
			}
