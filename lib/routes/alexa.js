...
app.intent('TellAmountIntent', function(request, response) {
	ingredientController.fetchIngredient(request.data.session.user.userId,
...
app.intent('RegisterIntent', function(request, response) {
	userController.registerAlexa(request.data.session.user.userId,
			request.data.request.intent.name.slots.Code.value,
...
app.intent('MakeIntent', function(request, response) {
	recipeController.applyRecipe(request.data.session.user.userId,
			request.data.request.intent.name.slots.RecipeName.value,
			function(err, success){
		if (err) {
			return response.say('Internal server error, could not complete task.');
		}
